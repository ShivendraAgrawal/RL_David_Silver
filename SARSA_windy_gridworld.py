...
def SARSA(Q_values, alpha, gamma, epsilon, terminal_states, windy_columns, n_rows, n_columns):
    ...
    while next_state not in terminal_states:
        ...
        current_reward, next_state = windy_action(current_state[0],
                                                  ...
                                                  windy_columns,
                                                  n_rows, n_columns)
        # Use the Q values to find the next action for the state
        # you would reach after the wind interference
        next_action = epsilon_greedy(Q_values, next_state, epsilon) # Q values not updated so far

        # Update the Q values after getting better estimates after 1-step look-ahead using SARSA
...
def main(rounds):
    ...
    run SARSA updates many times
    :param rounds: Number of episodes we want to run SARSA for
    :return: None (Prints all the results)
    '''

    # Initialize the grid world, Q-values and the parameters for SARSA
    ...
    # Running SARSA Q value updates for many episodes
    for i in range(rounds):
        Q_values = SARSA(Q_values, alpha, gamma, epsilon,
